...
export default function Reportes() {
  ...
  useEffect(() => {
    ...
    api.listarUsuarios({ token }).then(r => {
  ...
  const generar = async () => {
    const payload = {
      ...
      estado: filtro.estado_tramite || undefined,
    }
      ...
      <div style={S.card}>
        <div className="grid-four">
          ...
          {tipo !== 'tramites' ? (
            ...
            <div>
              <div style={S.label}>Estado tramite</div>
              <select style={S.input} value={filtro.estado_tramite} onChange={e => setFiltro(f => ({ ...f, estado_tramite: e.target.value }))}>
                <option value="">Todos</option>
        ...
        <div className="button-row" style={{ marginTop: 10 }}>
          ...
          <button onClick={exportar} style={S.btn}>Exportar archivo</button>
        </div>
      </div>
                ...
                <tr style={{ color: 'var(--text-faint)', textAlign: 'left' }}>
                  {tipo === 'tramites' ? (
                    <>
                      ...
                      <th style={{ padding: '6px 4px' }}>Saldo</th>
                    </>
                  ) : (
                    <>
                      <th style={{ padding: '6px 4px' }}>Fecha</th>
              ...
              <tbody>
                {rows.map(r => (
                  <tr key={r.id} style={{ borderTop: '1px solid var(--divider)' }}>
                    {tipo === 'tramites' ? (
                      <>
                        <td style={{ padding: '6px 4px' }}>{r.creado_en}</td>
                        <td style={{ padding: '6px 4px' }}>{r.tipo}</td>
                        <td style={{ padding: '6px 4px' }}>{r.nombre}</td>
                        <td style={{ padding: '6px 4px' }}>{r.estado}</td>
                        <td style={{ padding: '6px 4px' }}>{r.saldo ?? '-'}</td>
                      </>
                    ) : (
                      <>
