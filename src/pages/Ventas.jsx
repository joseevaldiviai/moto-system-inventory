...
export default function Ventas() {
  ...
  const load = async () => {
    const res = await window.api.listarProformas({ token, estado: 'ACTIVA' })
  ...
  const consolidar = async (id) => {
    ...
    const res = await window.api.crearVenta({ token, data: { proforma_id: id, tramites: tramitesPayload } })
    if (!res.ok) return toast.error(res.error || 'Error')
